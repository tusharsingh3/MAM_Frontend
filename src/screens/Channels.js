...
import { channels } from '../components/data';
...
function Channels() {
    ...
    const fetchRecordByKey = async (key) => {
        return dataSource.load()
            .then((data) => {
                const record = data.find((item) => item.ChannelId === key);
                if (record) {
                    return record;
                } else {
    ...
    return (
        <div>
            <GridComponent title='Channels' keyExpr='ChannelId' allowEdit={true} allowSelection={true} columns={columns} dataSource={dataSource} selectionChanged={selectionChanged} deleteRecords={deleteRecords} selectedItemKeys={selectedItemKeys} onChangesChange={updateData} />
        </div>
    );
};
